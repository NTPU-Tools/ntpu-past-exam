...
const CreatePostDialog = () => {
  ...
  function closeCreatePostDialog() {
    ...
    removeParams("open_create_post_dialog");
  }
  ...
  const onSubmit = async (values: z.infer<typeof createPostSchema>) => {
    try {
      ...
      formData.set("department_id", params.department_id as string);
      // @ts-ignore
  ...
  return (
    <Dialog
      open={get("open_create_post_dialog") === "true"}
      onOpenChange={(open) => {
        if (!open) {
          closeCreatePostDialog();
        }
        ...
        <DialogFooter>
          ...
          <Button
            onClick={form.handleSubmit(onSubmit, (e) => {
              toast({
                title: (e as any)?.[Object.keys(e)?.[0]]?.message ?? "表單發生錯誤",
                variant: "error",
              });
            })}
            disabled={isLoading}
